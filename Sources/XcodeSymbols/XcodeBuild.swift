...
public final class Xcodebuild {
    ...
    func build(project: XcodeProjectlike, scheme: String, allSchemes: [String], additionalArguments: [String] = [], buildForTesting: Bool = false) throws -> String {
        ...
        let args = [
            "-\(project.type)", "'\(project.path.path())'",
            "-scheme", "'\(scheme)'",
            "-parallelizeTargets",
            "-derivedDataPath", "'\(try derivedDataPath(for: project, schemes: allSchemes).path())'",
            "-quiet"
        ]
        let envs = [
    ...
    func removeDerivedData(for project: XcodeProjectlike, allSchemes: [String]) throws {
        try shell.exec(["rm", "-rf", try derivedDataPath(for: project, schemes: allSchemes).path()])
    }

    func indexStorePath(project: XcodeProjectlike, schemes: [String]) throws -> URL {
        ...
        let pathsToTry = ["Index.noindex/DataStore", "Index/DataStore"]
            .map { derivedDataPath.appending(path: $0) }
        guard let path = pathsToTry.first(where: {
            directoryExistsAtPath($0.path())
        }) else {
            // Change to error
            fatalError()
...
//            throw PeripheryError.jsonDeserializationError(error: error, json: jsonString)
    ...
    private func derivedDataPath(for project: XcodeProjectlike, schemes: [String]) throws -> URL {
        ...
        return try Constants.cachePath().appending(path: "DerivedData-\(xcodeVersionHash)-\(projectHash)-\(schemesHash)")
    }
